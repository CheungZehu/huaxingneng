<template lang="html">
  <div class="main">
    <div class="header">
      ...
      <div class="datetime" @click="visibility=true">
    ...
    <div class="dataBox">
      <ul>
        <li v-for="n in list">
          ...
          <div>
            <input type="number" v-model="n.web_price">
          </div>
        </li>
      </ul>
    ...
    <group>
      <datetime v-model="datetime" title="123" :show.sync="visibility" @on-change="change" v-show="false" :start-date="startDate"></datetime>
    </group>
  </div>
</template>
...
export default {
  ...
  methods:{
    router(r){
      ...
      localStorage.setItem('web_price',JSON.stringify(this.list))
  ...
  mounted(){
    ...
    self.startDate = dateFormat(date,'YYYY-MM-DD');
    // self.datetime = dateFormat(date,'YYYY-MM-DD');
